...
struct VectorToolbar: View {
    ...
    private func exportSelection() {
        ...
        // Euronav 5 builds a card structure (fast), then needs a destination
        // folder. The spinner is shown for the actual card write, after the
        // folder picker is dismissed — see writeEuronav5CardStructure.
        if format == .dmg {
            let service = Euronav5ExportService()
            pendingEuronav5Files = service.exportEuronav5Card(vectorLayers: layersToExport, to: .three)
            // Present after the export-format dialog dismisses — presenting the
            // folder picker in the same pass leaves it unresponsive.
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) {
                self.showEuronav5FolderPicker = true
            }
            return
        }

        // No progress spinner: these formats are saved through the document
        ...
        do {
            ...
            case .kmz:
                ...
                guard let kmlFile = files.first else { throw ExportError.nothingToExport }
            ...
            case .geojson:
                ...
                guard let file = files.first else { throw ExportError.nothingToExport }
            ...
            // Present after the export-format dialog dismisses — a UIDocumentPicker
            // presented in the same render pass as another transition is
            // unresponsive.
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) {
